...
from dynev4eo._src.viz.ts import plot_ts_station_pot, plot_scatter_station, plot_histogram_station, plot_density_station
from dynev4eo._src.inference import SVILearner, MCMCLearner
from dynev4eo._src.models.station.gpd import ProbGPDIID, ProbGPDIIDReparam
...
from dynev4eo._src.viz.returns import plot_return_level_gpd, plot_return_level_hist
import numpyro
...
def main(quantile: str="95", num_days: int=3, freq: str="D"):
    ...
    method_name = f"{extreme_method}_{model_dist}_{inference}_q{quantile}_d{num_days}{freq}"
    ...
    for istation_id in pbar:
        ...
        ds_magnitude = calculate_pot_extremes(ds_station, num_days=num_days, freq=freq, quantile=quantile_num)



        pbar.set_description(f"Station ID: {istation_id} | Plotting EDA...")
        plot_ts_station_pot(ds_station, ds_magnitude, fig_save_path)
        plot_scatter_station(ds_magnitude, fig_save_path)
        plot_histogram_station(ds_magnitude, fig_save_path)
        plot_density_station(ds_magnitude, fig_save_path)
        ...
        plot_qq_plot_gpd(arxiv_summary, fig_save_path) 
    

        # calculate return period
        pbar.set_description(f"Station ID: {istation_id} | Plot Return Level...")
        plot_return_level_gpd(arxiv_summary=arxiv_summary, figures_save_dir=fig_save_path)
